...
type GenerateReportRequestClass struct {
	...
	ExecuteQueryIdsOnly *[]string `json:"executeQueryIdsOnly" yaml:"executeQueryIdsOnly"`
	...
	FromTimestamp string `json:"fromTimestamp" yaml:"fromTimestamp"`

	// GroupByTime Optional per-run override of the time grouping period for this generation (manual / test). Same format as `ReportQueryPluginBaseParameters.groupByTimePeriod`: `X<m|h|d|w>` (e.g. `15m`, `1h`, `1d`, `1w`).
	...
	GroupByTime *string `json:"groupByTime" yaml:"groupByTime"`
	...
	SkipNotifications *bool `json:"skipNotifications" yaml:"skipNotifications"`
	...
	SkipWebhook *bool `json:"skipWebhook" yaml:"skipWebhook"`
	...
	ToTimestamp *string `json:"toTimestamp" yaml:"toTimestamp"`
}
...
type ReportQueryPluginBaseParameters struct {
	...
	// GroupByTime If set to TRUE then you get a break-down by time. The default interval is chosen from the query range (and typically matches your schedule length when the report is scheduled), e.g.:
